...
def RecognizePlateJson(image, type):
    ...
    for j,plate in enumerate(images):
        ...
        print("res",res)
        #
        # cv2.imwrite("./"+str(j)+"_rough.jpg",plate) ## 保存的图形1
        ...
        print("e2e:",model.recognizeOne(image_rgb)[0])
...
def SimpleRecognizePlateByE2E(image, type):
    t0 = time.time()
    images = detect.detectPlateRough(image,image.shape[0],top_bottom_padding_rate=0.1)
    res_set = []
    for j,plate in enumerate(images):
...
def SimpleRecognizePlate(image, type):
    t0 = time.time()
    images = detect.detectPlateRough(image,image.shape[0],top_bottom_padding_rate=0.1)
    res_set = []
    for j,plate in enumerate(images):
        ...
        cache.verticalMappingToFolder(image_rgb)
        model = e2e.e2e(type)
        print("e2e:", model.recognizeOne(image_rgb))
