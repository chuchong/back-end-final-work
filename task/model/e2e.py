...
class e2e:
    ...
    def __init__(self, type):
        '''
        :param type: 0 CNN; 1 GRU
        '''
        if type == 0:
            self.pred_model = model.construct_model(os.path.join(settings.MODEL_DIR,"ocr_plate_all_w_rnn_2.h5"))
            self.width = 160
            self.height = 40
        else:
            self.pred_model = grum.construct_model(os.path.join(settings.MODEL_DIR,"ocr_plate_all_gru.h5"))
            self.width = 164
            self.height = 48
